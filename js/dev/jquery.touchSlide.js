...
(function( $ ) {
    ...
    TouchSlideController.prototype = {
		...
		resized: function(){
			...
			if(this.options.vertical){
				...
				this.element.css({width: this.pageSize * (this.totalPages + 1)});
				this.pages.css({width: this.pageSize});
				this.pages.each(jQuery.proxy(this, "alignPages"));
		...
		alignPages: function(index){
			...
			if(this.options.vertical){
				this.pages.eq(index).css({top: this.pageSize * index});
			} else {
				this.pages.eq(index).css({left: this.pageSize * index});
			}
		...
		firstTouch:function(event){
			TweenLite.killTweensOf(this.element, true);
			this.stopAnimating();
			if(this.animating){
				return;
			} else {
		...
		resetMoveObject: function(obj){
			...
			this.stepTouch = {first:true, second:true};
			this.initialiseMoveObject(obj);
			
		},

		initialiseMoveObject: function(obj){
		...
		checkEnd: function(obj){
			...
			*/
			...
			if((this.currentIndex == 0 && distance > 0) || (this.currentIndex == this.totalPages && distance < 0)){
				...
				if(Math.abs(distance) >= (this.pageSize / 2) || speed >= this.options.speedThreshold){
					this.startAnimating();
		...
		moveScrolled:function(value){
			...
			this.animateElement(this.element, value, 0, "", Linear.easeNone);
		...
		resetToPosition:function(jump){
			...
			this.animateElement(this.element, (this.pageSize * -this.currentIndex), speed, jQuery.proxy(this, "stopAnimating"), Power2.easeOut);
		...
		/* ---------  */
		moveToPage:function(target){
			
			if(target == this.currentIndex) return;
			this.movingAdmin(target);
		...
		movingAdmin:function(target, speed){
			...
			this.animateElement(this.element, this.pageSize * -target, targetSpeed, jQuery.proxy(this, "pageArrived"));
		...
		animateElement: function(item, target, duration, callback, ease){
			...
			TweenMax.to(item, duration, {css:animateObject, onComplete:callback, ease:easing, onUpdate:jQuery.proxy(this, "updateAnimation")});
			
		},

		updateAnimation: function(){
			if(this.options.noisy) this.element.trigger("moving", {x:this.element.position().left});
		}
	...
	$.fn.touchSlide = function(settings) {
		return this.each(function() {
      		var touchSlideController = new TouchSlideController(this, settings);
    	});
